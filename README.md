...
Stack: Next.js (App Router) + TypeScript + Tailwind CSS v4. Services content is authored as Markdown in `content/services/*.md` and rendered via `src/lib/services.ts`.
...
- Replace `[PHONE]` and `[EMAIL]` placeholders in `src/lib/site-config.ts` with real contact info.
- Replace the gradient placeholders (Hero, Completed Works, About) with real project photos.
- Replace testimonial placeholders in `src/components/Testimonials.tsx` with real customer reviews.
